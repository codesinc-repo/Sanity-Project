...
import { useEventsCategories } from '@/lib/useEventsCategories';

import type { Locale } from '../../../../../i18n-config';

const getPageSettings = cache(async (page: string, lang: Locale) => {
	const getSeoVariant = lang === 'en' ? seoVariantEN : seoVariantSV;
	const [menuProps, pages, eventsCategories, seoVariant] = await Promise.all([
		menuSettings(),
		sitemapOptions(),
		useEventsCategories(lang),
		getSeoVariant(),
	]);
...
export async function generateMetadata({
	...
	const { data, seoVariant } = await getPageSettings(
		'kalender/instrumentpanelen/ny-aktivitet',
		params.lang,
	);
...
const Page = async ({ params }: { params: { lang: Locale } }) => {
	...
	const { data, seoVariant, eventsCategories, menuProps, pages } = await getPageSettings(
		'kalender/instrumentpanelen/ny-aktivitet',
		lang,
	);
	...
	return (
		<>
			...
			/>
			<PageBlocks blocks={blocks} lang={lang} eventsCategories={eventsCategories} />
