...
export async function generateMetadata({
	...
	const { data, seoVariant } = await getPageSettings(params.slug, params.lang);
...
const Page = async ({ params }: { params: { slug: string[] | string; lang: Locale } }) => {
	const { slug, lang } = params;
	const { data, seoVariant, menuProps, eventsResponse, pages } = await getPageSettings(
		slug,
		lang,
	);

	const informationClass = slug.includes('information');
			...
			{informationClass ? (
				<Container size={'xl'}>
					<Box className={'information-page'}>
						...
						<div className={'information-links-wrap'}>
							<Text className={'information-links-title'}>
								{'Gottsundakalendern länkar'}
							</Text>
							<Link href={`/${lang}/kalender/inloggning/`}>
								<span>Logga in</span>
								<IconExternalLink />
							</Link>
							<Link href={`/${lang}/kalender/glomt-losenordet/`}>
								<span>Skapa nytt konto</span>
								<IconExternalLink />
							</Link>
							<Link href={`/${lang}/kalender`}>
								<span>Gottsunda kalendern</span>
								<IconExternalLink />
							</Link>
						</div>
					</Box>
